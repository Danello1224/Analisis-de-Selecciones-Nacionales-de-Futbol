...
SELECT 
    ...
    SUM(CASE 
        WHEN local = 'CROATIA' AND goles_local > goles_visitante THEN 1
        WHEN visitante = 'CROATIA' AND goles_visitante > goles_local THEN 1
        ELSE 0
    END) AS victorias,
    ...
    SUM(CASE 
        WHEN local = 'CROATIA' THEN goles_local
        WHEN visitante = 'CROATIA' THEN goles_visitante
        ELSE 0
    ...
    SUM(CASE 
        WHEN local = 'CROATIA' THEN goles_visitante
        WHEN visitante = 'CROATIA' THEN goles_local
        ELSE 0
...
-- Filter only Croatia matches
-- Filtra únicamente partidos de Croacia
WHERE local = 'CROATIA'
   OR visitante = 'CROATIA'
