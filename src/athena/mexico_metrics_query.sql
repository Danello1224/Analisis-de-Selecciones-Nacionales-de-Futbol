...
SELECT 
    ...
    SUM(CASE 
        WHEN local = 'MEXICO' AND goles_local > goles_visitante THEN 1
        WHEN visitante = 'MEXICO' AND goles_visitante > goles_local THEN 1
        ELSE 0
    END) AS victorias,
    ...
    SUM(CASE 
        WHEN local = 'MEXICO' THEN goles_local
        WHEN visitante = 'MEXICO' THEN goles_visitante
        ELSE 0
    ...
    SUM(CASE 
        WHEN local = 'MEXICO' THEN goles_visitante
        WHEN visitante = 'MEXICO' THEN goles_local
        ELSE 0
...
-- Filter only Mexico matches
-- Filtra únicamente partidos de México
WHERE local = 'MEXICO'
   OR visitante = 'MEXICO';
