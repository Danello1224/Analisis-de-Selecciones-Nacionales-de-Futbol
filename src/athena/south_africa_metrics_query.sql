...
SELECT 
    ...
    SUM(CASE 
        WHEN local = 'SOUTH AFRICA' AND goles_local > goles_visitante THEN 1
        WHEN visitante = 'SOUTH AFRICA' AND goles_visitante > goles_local THEN 1
        ELSE 0
    END) AS victorias,
    ...
    SUM(CASE 
        WHEN local = 'SOUTH AFRICA' THEN goles_local
        WHEN visitante = 'SOUTH AFRICA' THEN goles_visitante
        ELSE 0
    ...
    SUM(CASE 
        WHEN local = 'SOUTH AFRICA' THEN goles_visitante
        WHEN visitante = 'SOUTH AFRICA' THEN goles_local
        ELSE 0
...
-- Filter only South Africa matches
-- Filtra únicamente partidos de Sudáfrica
WHERE local = 'SOUTH AFRICA'
   OR visitante = 'SOUTH AFRICA'
