...
SELECT 
    ...
    SUM(CASE 
        WHEN local = 'ENGLAND' AND goles_local > goles_visitante THEN 1
        WHEN visitante = 'ENGLAND' AND goles_visitante > goles_local THEN 1
        ELSE 0
    END) AS victorias,
    ...
    SUM(CASE 
        WHEN local = 'ENGLAND' THEN goles_local
        WHEN visitante = 'ENGLAND' THEN goles_visitante
        ELSE 0
    ...
    SUM(CASE 
        WHEN local = 'ENGLAND' THEN goles_visitante
        WHEN visitante = 'ENGLAND' THEN goles_local
        ELSE 0
...
-- Filter only England matches
-- Filtra únicamente partidos de Inglaterra
WHERE local = 'ENGLAND'
   OR visitante = 'ENGLAND'
