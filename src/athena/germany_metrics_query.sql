...
SELECT 
    ...
    SUM(CASE 
        WHEN local = 'GERMANY' AND goles_local > goles_visitante THEN 1
        WHEN visitante = 'GERMANY' AND goles_visitante > goles_local THEN 1
        ELSE 0
    END) AS victorias,
    ...
    SUM(CASE 
        WHEN local = 'GERMANY' THEN goles_local
        WHEN visitante = 'GERMANY' THEN goles_visitante
        ELSE 0
    ...
    SUM(CASE 
        WHEN local = 'GERMANY' THEN goles_visitante
        WHEN visitante = 'GERMANY' THEN goles_local
        ELSE 0
...
-- Filter only Germany matches
-- Filtra únicamente partidos de Alemania
WHERE local = 'GERMANY'
   OR visitante = 'GERMANY'
