...
SELECT 
    ...
    SUM(CASE 
        WHEN local = 'CANADA' AND goles_local > goles_visitante THEN 1
        WHEN visitante = 'CANADA' AND goles_visitante > goles_local THEN 1
        ELSE 0
    END) AS victorias,
    ...
    SUM(CASE 
        WHEN local = 'CANADA' THEN goles_local
        WHEN visitante = 'CANADA' THEN goles_visitante
        ELSE 0
    ...
    SUM(CASE 
        WHEN local = 'CANADA' THEN goles_visitante
        WHEN visitante = 'CANADA' THEN goles_local
        ELSE 0
...
-- Filter only Canada matches
-- Filtra únicamente partidos de Canadá
WHERE local = 'CANADA'
   OR visitante = 'CANADA'
