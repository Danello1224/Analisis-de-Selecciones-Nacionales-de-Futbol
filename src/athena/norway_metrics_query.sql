...
SELECT 
    ...
    SUM(CASE 
        WHEN local = 'NORWAY' AND goles_local > goles_visitante THEN 1
        WHEN visitante = 'NORWAY' AND goles_visitante > goles_local THEN 1
        ELSE 0
    END) AS victorias,
    ...
    SUM(CASE 
        WHEN local = 'NORWAY' THEN goles_local
        WHEN visitante = 'NORWAY' THEN goles_visitante
        ELSE 0
    ...
    SUM(CASE 
        WHEN local = 'NORWAY' THEN goles_visitante
        WHEN visitante = 'NORWAY' THEN goles_local
        ELSE 0
...
-- Filter only Norway matches
-- Filtra únicamente partidos de Noruega
WHERE local = 'NORWAY'
   OR visitante = 'NORWAY'
