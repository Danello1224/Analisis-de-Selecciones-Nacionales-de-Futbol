...
SELECT 
    ...
    SUM(CASE 
        WHEN local = 'BOSNIA & HERZEGOVINA' AND goles_local > goles_visitante THEN 1
        WHEN visitante = 'BOSNIA & HERZEGOVINA' AND goles_visitante > goles_local THEN 1
        ELSE 0
    END) AS victorias,
    ...
    SUM(CASE 
        WHEN local = 'BOSNIA & HERZEGOVINA' THEN goles_local
        WHEN visitante = 'BOSNIA & HERZEGOVINA' THEN goles_visitante
        ELSE 0
    ...
    SUM(CASE 
        WHEN local = 'BOSNIA & HERZEGOVINA' THEN goles_visitante
        WHEN visitante = 'BOSNIA & HERZEGOVINA' THEN goles_local
        ELSE 0
...
-- Filter only Bosnia & Herzegovina matches
-- Filtra únicamente partidos de Bosnia y Herzegovina
WHERE local = 'BOSNIA & HERZEGOVINA'
   OR visitante = 'BOSNIA & HERZEGOVINA'
